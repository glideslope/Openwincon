...
class DQN:
	...
	def __init__(self, session, data):
		...
		self.input_X = tf.placeholder(tf.float32, [None, self.NUM_UE + 1, self.NUM_AP, self.STATE_LEN])
	...
	def _build_network(self, name):
		with tf.variable_scope(name):
			...
			Q = tf.layers.dense(model, self.NUM_ACTION, activation=None)
	...
	def _build_op(self):
		...
		one_hot = tf.one_hot(self.input_A, self.NUM_ACTION, 1.0, 0.0)
	...
	def init_state(self, state):
		...
		# self.input_X = tf.placeholder(tf.float32, [None, NUM_UE + 1, NUM_AP, self.STATE_LEN])
	...
	def remember(self, state, action, reward, terminal):
		...
		next_state = np.reshape(state, (self.NUM_UE + 1, self.NUM_AP, 1))
